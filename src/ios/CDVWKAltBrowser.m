...
#import "CDVWKAltBrowser.h"
#import <Cordova/NSDictionary+CordovaPreferences.h>
#import <Cordova/CDVWebViewProcessPoolFactory.h>
#import <Cordova/CDVPluginResult.h>

#define    kAltBrowserTargetSelf @"_self"
#define    kAltBrowserTargetSystem @"_system"
#define    kAltBrowserTargetBlank @"_blank"

#define    kAltBrowserToolbarBarPositionBottom @"bottom"
#define    kAltBrowserToolbarBarPositionTop @"top"
...
#pragma mark CDVWKAltBrowser

@implementation CDVWKAltBrowser

static CDVWKAltBrowser* instance = nil;
...
{
    if (self.altBrowserViewController == nil) {
        NSLog(@"IAB.close() called but it was already closed.");
        return;
    }
    
    // Things are cleaned up in browserExit.
    [self.altBrowserViewController close];
}
...
- (void)open:(CDVInvokedUrlCommand*)command
{
    ...
    NSString* target = [command argumentAtIndex:1 withDefault:kAltBrowserTargetSelf];
    ...
    if (url != nil) {
        ...
        if ([self isSystemUrl:absoluteUrl]) {
            target = kAltBrowserTargetSystem;
        }
        
        if ([target isEqualToString:kAltBrowserTargetSelf]) {
            [self openInCordovaWebView:absoluteUrl withOptions:options];
        } else if ([target isEqualToString:kAltBrowserTargetSystem]) {
            [self openInSystem:absoluteUrl];
        } else { // _blank or anything else
            [self openInAltBrowser:absoluteUrl withOptions:options];
        }
...
- (void)openInAltBrowser:(NSURL*)url withOptions:(NSString*)options
{
    CDVAltBrowserOptions* browserOptions = [CDVAltBrowserOptions parseOptions:options];
    ...
    if (browserOptions.cleardata) {
        ...
        [dataStore removeDataOfTypes:[WKWebsiteDataStore allWebsiteDataTypes] modifiedSince:dateFrom completionHandler:^{
            ...
            self.altBrowserViewController.webView.configuration.processPool = [[WKProcessPool alloc] init]; // create new process pool to flush all data
        }];
    }
    ...
    if (self.altBrowserViewController == nil) {
        self.altBrowserViewController = [[CDVWKAltBrowserViewController alloc] initWithBrowserOptions: browserOptions andSettings:self.commandDelegate.settings];
        self.altBrowserViewController.navigationDelegate = self;
        
        if ([self.viewController conformsToProtocol:@protocol(CDVScreenOrientationDelegate)]) {
            self.altBrowserViewController.orientationDelegate = (UIViewController <CDVScreenOrientationDelegate>*)self.viewController;
        }
    }
    
    [self.altBrowserViewController showLocationBar:browserOptions.location];
    [self.altBrowserViewController showToolBar:browserOptions.toolbar :browserOptions.toolbarposition];
    if (browserOptions.closebuttoncaption != nil || browserOptions.closebuttoncolor != nil) {
        int closeButtonIndex = browserOptions.lefttoright ? (browserOptions.hidenavigationbuttons ? 1 : 4) : 0;
        [self.altBrowserViewController setCloseButtonTitle:browserOptions.closebuttoncaption :browserOptions.closebuttoncolor :closeButtonIndex];
    }
    ...
    self.altBrowserViewController.modalPresentationStyle = presentationStyle;
    ...
    self.altBrowserViewController.modalTransitionStyle = transitionStyle;
    
    //prevent webView from bouncing
    if (browserOptions.disallowoverscroll) {
        if ([self.altBrowserViewController.webView respondsToSelector:@selector(scrollView)]) {
            ((UIScrollView*)[self.altBrowserViewController.webView scrollView]).bounces = NO;
        } else {
            for (id subview in self.altBrowserViewController.webView.subviews) {
    ...
    [self.altBrowserViewController navigateTo:url];
...
- (void)show:(CDVInvokedUrlCommand*)command withNoAnimate:(BOOL)noAnimate
{
    ...
    __block CDVAltBrowserNavigationController* nav = [[CDVAltBrowserNavigationController alloc]
                                                        initWithRootViewController:self.altBrowserViewController];
    nav.orientationDelegate = self.altBrowserViewController;
    nav.navigationBarHidden = YES;
    nav.modalPresentationStyle = self.altBrowserViewController.modalPresentationStyle;
    nav.presentationController.delegate = self.altBrowserViewController;
    
    __weak CDVWKAltBrowser* weakSelf = self;
    
    // Run later to avoid the "took a long time" log message.
    dispatch_async(dispatch_get_main_queue(), ^{
        if (weakSelf.altBrowserViewController != nil) {
...
- (void)hide:(CDVInvokedUrlCommand*)command
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self.altBrowserViewController != nil) {
            [self.altBrowserViewController.presentingViewController dismissViewControllerAnimated:YES completion:nil];
        }
    });
}
...
{
    // Ensure a message handler bridge is created to communicate with the CDVWKAltBrowserViewController
...
- (void)evaluateJavaScript:(NSString *)script {
    ...
    [self.altBrowserViewController.webView evaluateJavaScript:script completionHandler:^(id result, NSError *error) {
        if (error == nil) {
            if (result != nil) {
                NSLog(@"%@", result);
            }
...
- (BOOL)isValidCallbackId:(NSString *)callbackId
{
    ...
    if (self.callbackIdPattern == nil) {
        self.callbackIdPattern = [NSRegularExpression regularExpressionWithPattern:@"^AltBrowser[0-9]{1,10}$" options:0 error:&err];
        if (err != nil) {
...
/**
 * The message handler bridge provided for the AltBrowser is capable of executing any oustanding callback belonging
 * to the AltBrowser plugin. Care has been taken that other callbacks cannot be triggered, and that no
...
//    self.altBrowserViewController.currentURL = theWebView.URL;
}
...
- (void)didFinishNavigation:(WKWebView*)theWebView
{
    if (self.callbackId != nil) {
        ...
        if(url == nil){
            if(self.altBrowserViewController.currentURL != nil){
                url = [self.altBrowserViewController.currentURL absoluteString];
            }else{
                url = @"";
            }
        }
...
- (void)browserExit
{
    ...
    [self.altBrowserViewController.configuration.userContentController removeScriptMessageHandlerForName:IAB_BRIDGE_NAME];
    self.altBrowserViewController.configuration = nil;
    
    [self.altBrowserViewController.webView stopLoading];
    [self.altBrowserViewController.webView removeFromSuperview];
    [self.altBrowserViewController.webView setUIDelegate:nil];
    [self.altBrowserViewController.webView setNavigationDelegate:nil];
    self.altBrowserViewController.webView = nil;
    
    // Set navigationDelegate to nil to ensure no callbacks are received from it.
    self.altBrowserViewController.navigationDelegate = nil;
    self.altBrowserViewController = nil;
...
#pragma mark CDVWKAltBrowserViewController

@implementation CDVWKAltBrowserViewController
...
- (id)initWithBrowserOptions: (CDVAltBrowserOptions*) browserOptions andSettings:(NSDictionary *)settings
{
    ...
    if (self != nil) {
        ...
        _settings = settings;
        self.webViewUIDelegate = [[CDVWKAltBrowserUIDelegate alloc] initWithTitle:[[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleDisplayName"]];
...
- (void)createViews
{
    ...
    BOOL toolbarIsAtBottom = ![_browserOptions.toolbarposition isEqualToString:kAltBrowserToolbarBarPositionTop];
...
- (void)showToolBar:(BOOL)show : (NSString *) toolbarPosition
{
    ...
    if (show) {
        ...
        if ([toolbarPosition isEqualToString:kAltBrowserToolbarBarPositionTop]) {
...
{
    NSString* statusBarStylePreference = [self settingForKey:@"AltBrowserStatusBarStyle"];
